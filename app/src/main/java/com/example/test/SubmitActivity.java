...
public class SubmitActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        taskContent = intent.getStringExtra("activityDescription");
        taskPoint = intent.getStringExtra("taskPoint");
        taskId = intent.getStringExtra("taskId");
        parentActivity = intent.getStringExtra("activity");
        int index = taskPoint.indexOf("经验值");
        String subTaskPoint = taskPoint.substring(0, index);

        taskRequirementTitleTV.setText(taskName);
        taskRequirementTV.setText(taskContent);

        taskScoreTV = findViewById(R.id.activity_score_Tv);
        taskScoreTV.setText("作业分值" + subTaskPoint + "分");
...
//                finish();
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        for(int i = 0; i < SelectFileActivity.uploadFilePaths.size(); i++){
                            File file = new File(SelectFileActivity.uploadFilePaths.get(i));
//                            builder.addFormDataPart("submit_file_" + (i+1), file.getName(),
//                                    MultipartBody.create(MediaType.parse(MimeTypeMap.getSingleton().getMimeTypeFromExtension(getSuffix(file))), file));
                            builder.addFormDataPart("submit_file_" + (i+1), file.getName(),
                                    MultipartBody.create(MediaType.parse("file/*"), file));
                        }
                        builder.addFormDataPart("teacherPhone", ClassTabActivity.teacherPhone);
                        builder.addFormDataPart("classId", ClassTabActivity.classId);
                        builder.addFormDataPart("className", ClassTabActivity.courseName);
                        builder.addFormDataPart("taskId", taskId);
                        builder.addFormDataPart("taskName", taskName);
                        builder.addFormDataPart("taskContent", answerET.getText().toString());
                        builder.addFormDataPart("studentPhone", MainActivity.phoneNumber);
                        RequestBody requestBody = builder.build();

                        Request request = new Request.Builder()
                                .url("http://47.98.236.0:8080/submittask")
                                .post(requestBody)
                                .build();
...
//                                                            if(parentActivity.equals("allFragment")){
//                                                                setResult(RESULT_OK, intent1);
//                                                            }else{
//                                                                setResult(RESULT_CANCELED, intent1);
//                                                            }
                                                        String temp_experience = MemberFragment.experTV.getText().toString();
                                                        MemberFragment.experTV.setText("当前获得" + String.valueOf(getExperience(temp_experience) + Integer.valueOf(responseBodyStr)) + "经验值");
                                                        setResult(RESULT_OK, intent1);
                                                        SelectFileActivity.uploadFilePaths.clear();
                                                        SelectFileActivity.uploadFiletypes.clear();
                                                        finish();
